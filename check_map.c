...
/*   Updated: 2024/01/28 10:38:31 by nbouhali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_player(t_cube *cube)
{
	...
	while(cube->map[++i])
	{
		...
		while (cube->map[i][++j])
		{
			if(cube->map[i][j] == 'W' || cube->map[i][j] == 'N' || cube->map[i][j] == 'E' || cube->map[i][j] == 'S')
			{
				if(count == 0)
				{
					cube->xp = j;
     ...
     while (cube->map[++i])
     {
		j = -1;
         while (cube->map[i][++j])
         {
			...
			{
				printf("Errorss in map\n");
				exit(1);
			}
			else if(cube->map[i][j] == '0' && ((cube->map[i - 1][j] == ' ') || cube->map[i - 1][j] == '\t'))
			{
				printf("Errorsss-----> in map\n");
				exit(1);
			}
			else if(cube->map[i][j] == '0' && ((cube->map[i][j - 1] == ' ') || cube->map[i][j - 1] == '\t'))
			{
				printf("Errors in map\n");
				exit(1);
			}
         }
     }
